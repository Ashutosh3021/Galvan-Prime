...
class TestRRFFusion:
    ...
    def test_fused_results_leq_top_k(self):
        ...
        vector_results = [_make_search_result(t, score=0.9 - i * 0.1) for i, t in enumerate(corpus_texts)]
    ...
    def test_deduplication_preserves_best_score(self):
        ...
        corpus_meta = [{"source": "x.pdf", "page": 1, "doc_id": "d1"}, {"source": "y.pdf", "page": 2, "doc_id": "d2"}]
...
class TestQueryEndpoint:
    ...
    async def test_query_happy_path(self, client: AsyncClient, auth_headers):
        ...
        with patch("api.routes.query.run_rag_chain", new_callable=AsyncMock, return_value=mock_result):
            resp = await client.post(
                "/query",
