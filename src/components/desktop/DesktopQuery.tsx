    ...
    <aside className="w-64 flex-shrink-0 border-r border-surface-container-high bg-surface-container-low flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1">
        ...
        {sessions.filter(s => !s.isSaved).map(s => (
          ...
          >
            {activeId === s.id && (
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-1/2 bg-primary-container rounded-r-full" />
            )}
...
const initialMessages: ChatMessage[] = [
  {
    id: '1',
    role: 'user',
    content: 'Summarize the latency issues reported in the Q3 network logs and cross-reference them with the updated routing protocols.',
    timestamp: 'Today, 14:02 UTC',
  },
  {
    id: '2',
    role: 'bot',
    content: 'Based on the ingested Q3 network logs and the current routing documentation, the latency spikes correlate strongly with the deployment of protocol <code>v2.4.1-alpha</code> in the US-East region.\n\nThe logs indicate an average latency increase of 45ms during peak loads. The routing protocols stipulate fallback mechanisms, but they were not triggered due to the threshold being set at 50ms.',
    citations: [
      { filename: 'Q3_Network_Logs_Raw.csv', reference: 'Lines 402-510', icon: 'description' },
      { filename: 'Routing_Protocols_v2.pdf', reference: 'Pg 14', icon: 'picture_as_pdf' },
    ],
    timestamp: 'Today, 14:02 UTC',
  },
  {
    id: '3',
    role: 'user',
    content: 'Can we adjust that threshold dynamically? Show me the specific code block responsible.',
    timestamp: 'Today, 14:03 UTC',
  },
];
  ...
  return (
    <div className="flex justify-start w-full gap-4">
      <div className="w-8 h-8 rounded-full bg-primary-container/20 border border-primary-container/40 flex items-center justify-center flex-shrink-0 mt-1">
        <span className="material-symbols-outlined text-primary-container icon-fill" style={{ fontSize: '18px' }}>psychology</span>
      </div>
      <div className="max-w-[85%] flex flex-col gap-3">
        <div className="bg-surface-container text-on-surface px-5 py-4 rounded-2xl rounded-tl-sm border border-surface-container-high shadow-sm">
          <div
            className="text-[16px] leading-relaxed text-on-surface/90 mb-4"
            dangerouslySetInnerHTML={{
              __html: message.content.replace(
                /<code>(.*?)<\/code>/g,
                '<code class="font-mono text-[14px] bg-[#05070A] px-1.5 py-0.5 rounded text-secondary-container border border-surface-container-highest">$1</code>'
              ),
            }}
          />
        ...
        <div className="flex items-center gap-2 px-1">
          <button className="text-on-surface-variant hover:text-primary-container transition-colors p-1" title="Good response">
            <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>thumb_up</span>
          </button>
          <button className="text-on-surface-variant hover:text-error transition-colors p-1" title="Bad response">
            <span className="material-symbols-outlined" style={{ fontSize: '18px' }}>thumb_down</span>
          </button>
          <button className="text-on-surface-variant hover:text-on-surface transition-colors p-1 ml-2" title="Copy text">
...
function TypingIndicator() {
  return (
    <div className="flex justify-start w-full gap-4">
      <div className="w-8 h-8 rounded-full bg-primary-container/20 border border-primary-container/40 flex items-center justify-center flex-shrink-0 mt-1 shadow-[0_0_10px_rgba(255,102,0,0.2)]">
        <span className="material-symbols-outlined text-primary-container icon-fill animate-spin" style={{ fontSize: '18px' }}>sync</span>
      </div>
      <div className="bg-surface-container text-on-surface-variant px-5 py-3 rounded-2xl rounded-tl-sm border border-surface-container-high flex items-center gap-2">
        <div className="flex space-x-1">
          {[0, 150, 300].map(delay => (
            <div
              key={delay}
              className="w-2 h-2 bg-primary-container/60 rounded-full animate-bounce"
...
export default function DesktopQuery() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setTimeout(() => {
      const botMsg: ChatMessage = {
        ...
        role: 'bot',
        content: 'Based on the retrieved context, I can provide the following analysis...',
        timestamp: new Date().toISOString(),
      };
  ...
  return (
    <div className="bg-surface-container-lowest text-on-surface font-sans h-screen flex flex-col overflow-hidden">
      <DesktopTopNav />
        ...
        <main className="flex-1 flex flex-col bg-surface relative">
          ...
          <div className="h-14 border-b border-surface-container-high flex items-center justify-between px-6 bg-surface-container/30 backdrop-blur-sm z-10 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[14px] font-mono text-secondary-container bg-secondary-container/10 px-2 py-0.5 rounded border border-secondary-container/20">
                {activeSession}
              </span>
            </div>
            ...
            <div className="max-w-4xl mx-auto relative group">
              ...
              <form onSubmit={handleSubmit} className="relative bg-surface-container border border-surface-container-high rounded-xl shadow-lg flex flex-col focus-within:border-primary-container/50 transition-colors">
                ...
                <div className="flex justify-between items-center p-3 mt-1">
                  <div className="flex gap-2 text-on-surface-variant">
                    <button type="button" className="p-1.5 rounded hover:bg-surface-bright transition-colors" title="Attach file context">
                      <span className="material-symbols-outlined" style={{ fontSize: '20px' }}>attach_file</span>
                    </button>
                    <button type="button" className="p-1.5 rounded hover:bg-surface-bright transition-colors" title="Adjust retrieval settings">
                  ...
                  <button
                    ...
                    className="bg-primary-container hover:bg-[#ff7a1f] text-white px-5 py-2 rounded-lg text-[12px] font-semibold tracking-[0.05em] flex items-center gap-2 transition-all shadow-[0_0_15px_rgba(255,102,0,0.15)] hover:shadow-[0_0_20px_rgba(255,102,0,0.3)] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Ask
              ...
              <div className="text-center mt-2">
                <span className="text-[12px] font-semibold text-on-surface-variant/50">
                  GalvanR.A.G. v1.0.4 — Responses are synthesized from retrieved context.
                </span>
              </div>
            </div>
          </div>
