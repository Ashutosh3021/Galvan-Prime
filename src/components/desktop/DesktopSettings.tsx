        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-8 flex flex-col gap-6">
            ...
            <div className="bg-surface-container rounded-lg border border-surface-container-high p-6 flex flex-col gap-6 relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  return (
                    <label key={opt.id} className={`cursor-pointer relative rounded-lg border p-4 flex flex-col gap-2 transition-all ${
                      isSelected ? 'border-primary bg-primary/5 hover:bg-primary/10' : 'border-surface-container-highest bg-surface hover:bg-surface-variant/50'
                    }`}>
                      <div className="flex items-center justify-between">
                        <span className="text-[16px] font-bold text-on-surface">{opt.name}</span>
                        <input type="radio" name="llm_provider" value={opt.id} checked={isSelected} onChange={() => setField('llmProvider', opt.id)} className="hidden" />
                        <span className={`material-symbols-outlined ${isSelected ? 'text-primary' : 'text-on-surface-variant'}`} style={{ fontVariationSettings: isSelected ? "'FILL' 1" : "'FILL' 0" }}>
            ...
            <div className="bg-surface-container rounded-lg border border-surface-container-high p-6 flex flex-col gap-6 relative overflow-hidden">
              ...
              <div>
                <h2 className="text-[20px] font-semibold text-on-surface flex items-center gap-2">
                  <span className="material-symbols-outlined text-secondary-container">database</span>
                  Vector Database
                </h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { id: 'chroma',   name: 'ChromaDB', badge: 'Local' },
                  { id: 'pinecone', name: 'Pinecone',  badge: 'Cloud' },
                ].map(db => {
                  const isSelected = form.vectorDB === db.id;
                  return (
                    <label key={db.id} className={`cursor-pointer relative rounded-lg border p-4 flex flex-col gap-2 transition-all ${
                      isSelected ? 'border-secondary bg-secondary/5 hover:bg-secondary/10' : 'border-surface-container-highest bg-surface hover:bg-surface-variant/50'
                    }`}>
                      <div className="flex items-center justify-between">
                        <span className="text-[16px] font-bold text-on-surface">{db.name}</span>
                        <span className={`text-[12px] font-semibold ${isSelected ? 'bg-secondary/20 text-secondary-container' : 'bg-surface-container-lowest text-on-surface-variant'} px-2 py-1 rounded`}>{db.badge}</span>
                        <input type="radio" name="vector_db" value={db.id} checked={isSelected} onChange={() => setField('vectorDB', db.id)} className="hidden" />
                        <span className={`material-symbols-outlined ${isSelected ? 'text-secondary-container' : 'text-on-surface-variant'}`} style={{ fontVariationSettings: isSelected ? "'FILL' 1" : "'FILL' 0" }}>
                          {isSelected ? 'radio_button_checked' : 'radio_button_unchecked'}
                        </span>
                      </div>
                    </label>
            ...
            <div className="bg-surface-container rounded-lg border border-surface-container-high p-6 flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-8 mt-4">
                <div className="flex items-center justify-between text-[14px]">
                  <span className="text-secondary font-bold">Dense Vector (Semantic)</span>
          ...
          <div className="md:col-span-4 flex flex-col gap-6">
            <div className="bg-surface-container rounded-lg border border-surface-container-high p-6">
              ...
              <ul className="flex flex-col gap-4">
                {envVars.map(ev => (
                  <li key={ev.key} className={`flex items-center justify-between p-3 rounded bg-[#05070A] border ${ev.loaded ? 'border-surface-container-highest' : 'border-error/30 border-dashed'}`}>
                    <div className="flex items-center gap-3">
                      <span className={`material-symbols-outlined ${ev.loaded ? 'text-secondary-container' : 'text-error'}`} style={{ fontSize: '20px', fontVariationSettings: "'FILL' 1" }}>
                        {ev.loaded ? 'check_circle' : 'error'}
                      </span>
                      <span className="font-mono text-[14px] text-on-surface">{ev.key}</span>
                    </div>
                    <span className={`text-[12px] font-semibold tracking-[0.05em] ${ev.loaded ? 'text-secondary-container bg-secondary-container/10' : 'text-error bg-error/10'} px-2 py-1 rounded`}>
              ...
              <div className="mt-6 pt-6 border-t border-surface-variant text-[14px] text-on-surface-variant flex gap-3">
                ...
                <p>Changes to environment variables may require a service restart to take full effect across worker nodes.</p>
              </div>
            </div>
          </div>
        </div>
